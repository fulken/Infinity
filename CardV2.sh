...
# Function to wait for cooldown period with countdown in minutes or seconds
wait_for_cooldown() {
    cooldown_seconds="$1"
    start_time=$(date +%s)
...
choose() {
    ...
    # Check if current cooldown is more than 2 hours (7200 seconds)
    if [ "$cooldown" -gt 7200 ]; then
        echo "$next_item_id"
    elif [[ -z "$cooldown" || "$cooldown" -eq 0 ]]; then
        echo "$best_item_id"
