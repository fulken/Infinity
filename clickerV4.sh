...
echo -en "${green}Enter User-Agent [${yellow}Example: Mozilla/5.0...${green}]:${rest} "
read -r UserAgent

# Prompt for Proxy
echo -en "${green}Enter Proxy [${yellow}Example: socks5://user:pass@host:port${green}]:${rest} "
read -r Proxy

# Display IP address after connecting to the proxy
echo -e "${blue}Connecting to proxy and retrieving IP...${rest}"
IP=$(curl -s --proxy "$Proxy" https://api.ipify.org)
echo -e "${green}Your IP after proxy: ${yellow}$IP${rest}"
